...
namespace hermezcs.UnitTests
{
    public class SignatureTests : BaseTest
    {
        ...
        [Fact]
        public void Signature_Testing2_ShouldVerify()
        {
            // construct request
            var addresses = new Addresses();
            var bjjPubComp = addresses.HexToBase64BJJ(BJJ_DEV_PRIVATE_KEY);
            ...
            var msgBytes = Encoding.UTF8.GetBytes(METAMASK_MESSAGE);
            var signer = new EthereumMessageSigner();

            // take ETH privateKey bytes to get public key/addy
            var ethEcKey = new EthECKey(ETH_DEV_PRIVATE_KEY_STR);
            var pubAddy = ethEcKey.GetPublicAddress();                                          //+""

            // V1: Hash in, hash out
            var signatureV1 = signer.HashAndSign(METAMASK_MESSAGE, privateKeyEth);              // ""
            var recPubAddyV1 = signer.HashAndEcRecover(METAMASK_MESSAGE, signatureV1);          //+""
